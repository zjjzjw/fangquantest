...
class DeveloperProjectWebService
{
    /**
     * 项目列表页
        ...
        if (isset($developer_project_entity)) {
            ...
            if ($developer_id) {
                ...
                if (isset($developer_entity)) {
                    ...
                    $logo_id = $developer_entity->logo;
                    $resource_repository = new ResourceRepository();
                    $resource_entities = $resource_repository->getResourceUrlByIds($logo_id);
                    $resource_entity = current($resource_entities);
                    $developer_info['developer_logo_url'] = '/www/images/provider/default_logo.png';
                    if (isset($resource_entity)) {
                        $developer_info['developer_logo_url'] = $resource_entity ?
                            $resource_entity->url : '/www/images/provider/default_logo.png';
                    }
            ...
            $developer_project_category_type_array = DeveloperProjectCategoryType::acceptableEnums(); //项目类型
            ...
            $developer_project_type_array = DeveloperProjectType::acceptableEnums(); //项目类别
            ...
            //新的项目类型 多选
            $project_category_type_names = [];
            foreach ($developer_project_entity->project_categories as $project_category_type) {
                $project_category_type_names[] = $developer_project_category_type_array[$project_category_type] ?? '';
            }
            $data['project_category_type_names'] = implode('、', $project_category_type_names);
            ...
            //招标类型
            $developer_project_bidding_types = DeveloperProjectBiddingType::acceptableEnums();
            $data['developer_project_bidding_type_name'] = $developer_project_bidding_types[$developer_project_entity->bidding_type] ?? '';
            //项目招标品类
            $project_category_service = new ProjectCategoryService();
            $data['project_category_names'] = $project_category_service->getProjectCategoryNameByIds($developer_project_entity->project_category_ids);
        }
        $length_name = 0;
        $length_name = mb_strlen($data['name']);

        if ($length_name > 34) {
            $data['title'] = mb_substr($data['name'], 0, 34) . "...";
        } else {
